...
found:
  ...
  memset(&p->context, 0, sizeof(p->context));//clean
  p->context.ra = (uint64)forkret;
  p->context.sp = p->kstack + PGSIZE;
  p->usyspage->pid = p->pid;//PA
...
proc_pagetable(struct proc *p)
{
  ...
  //map a usyscall page below TRAPFRAME && TRAMPOLINE
...
uint64 pgaccess(void *pg, int number, void *store)
{
    ...
    for (int i = 0; i < number; i++) {
        ...
        if (pte !=0 && ((*pte)&PTE_A)) {
            ans |= 1 << i;
            *pte ^= PTE_A;// clear PTE_A, count back after operations
        }
    }
    //copyout
